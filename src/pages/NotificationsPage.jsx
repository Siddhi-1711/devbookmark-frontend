...
function buildMessage(n) {
  ...
  switch (n.type) {
    ...
    case "COMMENTED_RESOURCE":
      return `${actor} commented on ${title}`;
    case "REPOSTED_RESOURCE":
      return `${actor} reposted ${title}`;
    default:
...
function NotificationRow({ n, onRead, onDelete, busy }) {
  ...
  return (
    <div
      className={`rounded-xl border p-3 sm:p-4 transition group ${
        n.read
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-start gap-2">
            <div className="min-w-0 flex-1">
              ...
              {n.resourceId && (
                <Link
                  to={`/resources/${n.resourceId}`}
                  className="inline-block mt-1 text-blue-400 hover:text-blue-300 text-xs sm:text-sm transition"
                >
                  View →
                </Link>
              )}
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2 sm:gap-3">
            ...
            {n.actorId && (
              <Link
                to={`/users/${n.actorId}`}
                className="text-gray-500 hover:text-white text-xs transition break-all"
              >
                @{n.actorName}
              </Link>
            )}
          ...
          {/* Actions */}
          <div className="mt-3 flex flex-wrap items-center gap-2 sm:opacity-0 sm:group-hover:opacity-100 transition">
            {!n.read && (
              <button
                onClick={onRead}
                disabled={busy}
                className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-gray-800 text-gray-300 hover:text-white text-xs transition disabled:opacity-50"
              >
                <Check size={12} />
                Read
              </button>
            ...
            <button
              onClick={onDelete}
              disabled={busy}
              className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-gray-800 text-gray-300 hover:text-red-400 text-xs transition disabled:opacity-50"
            >
              <Trash2 size={12} />
              Delete
            </button>
    ...
    <AppLayout maxWidth="max-w-4xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
        <div className="min-w-0">
          <h1 className="text-white text-2xl sm:text-3xl font-bold flex flex-wrap items-center gap-2">
            <Bell size={20} className="text-blue-400 shrink-0" />
            <span>Notifications</span>
            {unreadCount > 0 && (
              <span className="text-xs sm:text-sm px-2 py-0.5 bg-blue-600 rounded-full font-medium">
                {unreadCount}
              </span>
            )}
          </h1>
        ...
        <div className="flex w-full sm:w-auto gap-2">
          <button
            onClick={load}
            className="flex-1 sm:flex-none p-2.5 rounded-lg bg-gray-900 border border-gray-800 text-gray-400 hover:text-white hover:border-gray-700 transition flex items-center justify-center"
            title="Refresh"
          >
            <RefreshCw size={15} />
          </button>
          ...
          <button
            ...
            className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-sm transition disabled:opacity-40"
          >
            <CheckCheck size={15} />
      ...
      {loading ? (
        <div className="space-y-3">
          {[...Array(6)].map((_, i) => (
            <div
              key={i}
              className="h-24 sm:h-20 bg-gray-900 border border-gray-800 rounded-xl animate-pulse"
            />
          ))}
        </div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-16 sm:py-20 px-4 bg-gray-900 border border-gray-800 rounded-xl">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mt-8 pt-6 border-t border-gray-800">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 sm:flex-none px-4 py-2 rounded-lg bg-gray-900 border border-gray-800 text-gray-300 hover:text-white disabled:opacity-40 transition text-sm"
            >
              ← Prev
            </button>
            <button
